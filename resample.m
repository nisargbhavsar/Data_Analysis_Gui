...
if(sys == 2) %Optotrak
    varargin = cell2mat(varargin);
    time = varargin(:,1)'; 
    lost_data_index = find(isnan(varargin(:,2)));
    lost_data_palm = find(isnan(varargin(:,5)));
    status = zeros(1,2);
    if((length(lost_data_index) / length (varargin(:,1)) > 0.3))
         status(1) = 1; %More than 30% of data missing
    end
    if((length(lost_data_palm) / length(varargin(:,1)) > 0.3))
        status (2) = 1; %more than 30% of data missing
    end
    if(length(lost_data_index) == length(varargin(:,1)))
        status(1) = 2; %Index data is entirely missing
    end
