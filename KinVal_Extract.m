...
if (type ==1 && system ==1) %Grasping motion Leap
    ...
    % Index finger max vector velocity
    if_max_v_vel_index = max(vector_vel(varargin(1,6):varargin(1,7),2)); %Peak Index Vector Vel look between start and end of target approach phase
    if_max_v_vel_index = find(vector_vel(:,1) == if_max_v_vel_index);
    
    %Thumb max vector velocity
    th_max_v_vel = max(vector_vel(varargin(1,6):varargin(1,7),3)); %Peak Thumb Vector Vel look between start and end of target end phase
    th_max_v_vel_index = find(vector_vel(:,2) == th_max_v_vel);
    
    %index finger max velocity (Z)
    if_max_vel_z = max(velocity(varargin(1,6):varargin(1,7),3));
    if_max_vel_index_z = find(velocity(varargin(1,6):varargin(1,7),3) == if_max_vel_z);

    %thumb max velocity (Z)
    th_max_vel_z = max(velocity(varargin(1,6):varargin(1,7), 6));
    th_max_vel_index_z = find(velocity(varargin(1,6):varargin(1,7),6) == th_max_vel_z);
...
%     end
    ...
    %%Return Phase
    % Index finger max vector velocity
    if_max_v_vel_index = min(vector_vel(varargin(1,8):varargin(1,9),2)); %Peak Index Vector Vel look between start and end of target approach phase
    if_max_v_vel_index = find(vector_vel(:,1) == if_max_v_vel_index);
    
    %Thumb max vector velocity
    th_max_v_vel = min(vector_vel(varargin(1,8):varargin(1,9),3)); %Peak Thumb Vector Vel look between start and end of target end phase
    th_max_v_vel_index = find(vector_vel(:,2) == th_max_v_vel);
    
    %index finger max velocity (Z)
    if_max_vel_z = min(velocity(varargin(1,8):varargin(1,9),3));
    if_max_vel_index_z = find(velocity(varargin(1,8):varargin(1,9),3) == if_max_vel_z);
    
    %Begining of object placement
    objp_begin = if_max_vel_index_z;
    while(abs(velocity(objp_begin,3)) < 0.1 && objp_begin >0)
    ...
    while(abs(velocity(objp_end,3))< 0.02 && objp_end < rows)
        objp_end = objp_end +1;
    end

    %thumb max velocity (Z)
    th_max_vel_z = min(velocity(varargin(1,8):varargin(1,9), 6));
    th_max_vel_index_z = find(velocity(varargin(1,8):varargin(1,9),6) == th_max_vel_z);
...
%     end
    ...
    %Grasping start
    %Grip aperture should be greater than 1/2 of the object diameter
    objective_f_ga = zeros(length(pos(:,1)),1);
    obj_dia = varargin(1,3)*1/2;
